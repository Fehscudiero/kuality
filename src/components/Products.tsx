...
function SectionGeometry() {
  ...
  return (
    ...
    >
      <div
        ref={shapeRef}
        className="absolute top-[-3%] right-0 w-[140%] h-[120%] bg-cyan-200"
        style={{
...
const NavButton = ({
  ...
  <div className="relative flex items-center">
    <button
      onClick={onClick}
      className={`group relative transition-all duration-500 rounded-2xl border-2 w-full p-11 z-20 ${
        active
          ? "bg-slate-900 border-cyan-500 shadow-2xl scale-[1.05]"
          : "bg-cyan-50/50 border-cyan-100 text-slate-600 hover:border-cyan-200 hover:bg-white"
      }`}
    >
      <div
      ...
      >
        <span
          className={`text-[10px] font-black uppercase tracking-widest mb-1 ${active ? "text-cyan-400" : "text-cyan-600"}`}
        >
          REF: {product.id.split("-")[1] || "01"}
        </span>
        <span
    ...
    {active && (
      <div
        className={`absolute top-1/2 -translate-y-1/2 h-[2px] bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.8)] z-10 transition-all duration-500 ${
...
export default function Products() {
  ...
  return (
    <section
      id="produtos"
      className="py-24 md:py-22 bg-white relative overflow-hidden"
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-2">
        <header className="mb-16 md:mb-28 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="max-w-2xl text-center md:text-left mx-auto md:mx-0">
            ...
            <h2 className="text-5xl md:text-4xl font-black text-slate-950 tracking-tighter uppercase italic leading-[0.85]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-10 items-center relative">
          ...
          <div className="lg:col-span-6 -mx-6 md:mx-0 relative">
            <button
              onClick={() => navigate(1)}
              className="lg:hidden absolute -right-2 top-1/2 -translate-y-1/2 w-12 h-32 bg-cyan-600 rounded-l-3xl z-[60] flex items-center justify-center shadow-2xl animate-pulse"
            >
            ...
            <div
              ...
              className="bg-slate-950 md:rounded-[8rem] text-white relative shadow-2xl border-y md:border border-white/5 min-h-[680px] md:min-h-[600px] flex flex-col justify-center overflow-hidden"
            >
              ...
              >
                <header className="mb-2">
                  ...
                  <h3 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-none mb-6">
                ...
                <div className="lg:hidden flex gap-1.5 mt-12 justify-center">
                  {products.map((_, idx) => (
                    <div
                      key={idx}
                      className={`h-1 flex-1 rounded-full transition-all duration-500 ${currentIndex === idx ? "bg-cyan-500 shadow-[0_0_10px_cyan]" : "bg-white/10"}`}
                    />
                  ))}
                </div>
              </div>

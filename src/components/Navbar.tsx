...
export default function Navbar({ scrollToSection }: NavbarProps) {
  ...
  const updateIndicator = useCallback((sectionId: string) => {
    ...
    const activeItem = navRef.current.querySelector(`[data-section="${sectionId}"]`) as HTMLElement
  ...
  useEffect(() => {
    ...
    return () => {
      clearTimeout(timer)
      window.removeEventListener('resize', () => updateIndicator(activeSection))
    }
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-18 lg:h-20">
          <a href="#hero" onClick={(e) => handleNavClick('hero', e)} className="flex items-center">
            <img 
